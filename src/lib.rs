...
pub mod themoviedb_api {
    ...
    impl fmt::Debug for TheMovieDB {
        fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
            write!(f,
                   "TheMovieDB => ApiKey: {}, BaseURL: {}, ImageBaseURL: {}",
                   self.api_key,
                   self.base_url,
                   self.image_base_url)
        }
    ...
    impl TheMovieDB {
        ...
        pub fn new(themoviedb_api_key: &'static str) -> Result<TheMovieDB, String> {
            let mut new_instance = TheMovieDB {
            ...
            let configuration_data = try!(new_instance.get_json_data_for("/configuration"
                                                                             .to_string()));
            ...
            new_instance.image_base_url = json_obj.get("images")
                                                  .unwrap()
                                                  .as_object()
                                                  .unwrap()
                                                  .get("secure_base_url")
        ...
        pub fn search(&self,
                      ...
                      -> Result<Vec<TheMovieDBItem>, String> {
            ...
            loop {
                // download data
                let search_data = try!(self.get_json_data_for(format!("/search/multi?query={}&p\
                                                                       age={}",
                                                                      search_term,
                                                                      page)));
                ...
                let search_totals = match TheMovieDB::get_object_field_as_u64(json_object,
                                                                              "total_results") {
                    Some(r) => r as usize,
                ...
                let page_totals = match TheMovieDB::get_object_field_as_u64(json_object,
                                                                            "total_pages") {
                    Some(r) => r as usize,
                ...
                for search_result in search_results {
                    ...
                    new_movie.original_title =
                        TheMovieDB::get_object_field_as_string(result_object, "original_title");
                    new_movie.title = TheMovieDB::get_object_field_as_string(result_object,
                                                                             "title");
        ...
        fn get_object_field_as_string(json_object: &Object,
        ...
        fn get_url_for(&self, url_string: String) -> Result<hyper::Url, String> {
            if url_string.contains("?") {
                let result_url = match hyper::Url::parse(&format!("{}{}&api_key={}",
                                                                  self.base_url,
                                                                  url_string,
                                                                  self.api_key)) {
                    Ok(u) => u,
